...
const WhatsAppIntegration = () => {
  ...
  // Helper to get correct amount (moved up to fix usage-before-declaration error)
  const getInvoiceAmount = useCallback((invoice: any) => {
    return (invoice.finalAmount && invoice.finalAmount > 0)
      ? invoice.finalAmount
  ...
  const handleSendInvoice = useCallback((invoice: any) => {
    ...
    // Only allow sending if a real PDF URL exists
    const invoiceUrl = invoice.pdf_url || invoice.pdfUrl || null;

    if (!invoiceUrl) {
      toast({
        title: "No PDF Found",
        description: "Invoice PDF link not found. Please generate and save the invoice PDF before sending via WhatsApp.",
        variant: "destructive",
      });
      return;
    }

    // Use robust helper for amount
    const amount = getInvoiceAmount(invoice);

    const messageText = `Hello ${invoice.customerName}, thank you for your business! Your invoice (${invoice.invoiceNumber}) for ${amount.toFixed(2)} is ready. You can view it here: ${invoiceUrl}`;
  ...
  }, [formatPhoneNumber, toast, getInvoiceAmount]);
                        ...
                        <tbody>
                          {invoices.map((invoice) => (
                            <tr key={invoice.id} className="border-b">
                              ...
                              {/* Use helper to get the correct amount */}
                              <td className="p-4">${getInvoiceAmount(invoice).toFixed(2)}</td>
